...
نسخهٔ پیاده‌سازی‌شده علاوه بر معیارهای MVP، پاسخ streaming، هویت ثابت MILO، تم روشن/تیره، تاریخچهٔ PostgreSQL، جهت متن فارسی/انگلیسی، رندر Markdown، پیوست تصویر/فایل متنی و لوگوی اختصاصی را نیز دارد.

هدف MVP این است که کاربر بتواند پیام بفرستد، پاسخ مدل را دریافت کند و مکالمه را در چند نوبت ادامه دهد. PostgreSQL به‌عنوان ارتقای محلی برای تاریخچه اضافه شده است؛ ورود کاربران، ابزارها، RAG و Agent loop همچنان خارج از محدوده‌اند.
...
Next.js browser ──> Next.js Route Handlers ───> Provider API ──> Model
                         │
                         └─────────────────────> PostgreSQL
```
...
PostgreSQL محلی با فرمان **docker compose up -d postgres** از ریشهٔ پروژه اجرا می‌شود. جزئیات کامل در [README وب](apps/web/README.md) آمده است.
